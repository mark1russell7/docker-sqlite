...
export function getMigrationStatus(db, migrations) {
    ...
    const applied = query(db, "SELECT version FROM _migrations ORDER BY version");
...
export function runMigrations(db, migrations) {
    ...
    for (const migration of status.pendingMigrations) {
        try {
            ...
            execute(db, "INSERT INTO _migrations (version, description) VALUES (?, ?)", [migration.version, migration.description]);
            applied.push(migration);
        }
        catch (error) {
            throw new Error(`Failed to apply migration ${migration.version} (${migration.description}): ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return applied;
}
 ...
 */
export function rollbackMigration(db, migrations) {
    const status = getMigrationStatus(db, migrations);
    ...
    if (!migration.down) {
        throw new Error(`Migration ${migration.version} (${migration.description}) has no down migration`);
    }
    try {
 ...
 */
export function createMigration(version, description, up, down) {
    const migration = { version, description, up };
    if (down !== undefined) {
        migration.down = down;
    }
    return migration;
